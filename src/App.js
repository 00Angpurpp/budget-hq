...
const path = `M ${CX + RO * Math.cos(s)} ${CY + RO * Math.sin(s)} A ${RO} ${RO} 0 ${lg} 1 ${CX + RO * Math.cos(e)} ${CY + RO * Math.sin(e)} L ${CX + RI * Math.cos(e)} ${CY + RI * Math.sin(e)} A ${RI} ${RI} 0 ${lg} 0 ${CX + RI * Math.cos(s)} ${CY + RI * Math.sin(s)} Z`;
return { …d, path };
});
      ...
      return (
        <div key={i} style={{ ...rowStyle, borderBottom: i < enrichedAbb.length - 1 ? `1px solid ${C.navy}44` : "none" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 10, color: C.grey }}>ogni {ab.giorno} del mese · prossimo <strong style={{ color: u.col }}>{fmtDate(ab.next)}</strong></div>
          </div>
  ...
  <div style={{ ...card(), padding: 0, overflow: "hidden" }}>
    ...
    {showFormRata && (
      <div style={{ padding: "13px 14px", borderBottom: `1px solid ${C.navy}66`, background: `${C.gold}08`, display: "flex", flexDirection: "column", gap: 8 }}>
        <input type="text" placeholder="Nome acquisto" value={formRata.nome} onChange={e => setFormRata(p => ({ ...p, nome: e.target.value }))} style={{ background: `${C.navy}66`, border: `1px solid ${C.gold}33`, borderRadius: 8, padding: "8px 10px", color: C.ivory, fontSize: 12, outline: "none", width: "100%" }} />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          {[["importo","Importo rata €","number"],["giorno","Giorno addebito","number"],["rateTot","N. rate totali","number"],["ratePagate","Rate già pagate","number"]].map(([k, label, type]) => (
            <div key={k}>
              <div style={{ fontSize: 9, color: C.grey, marginBottom: 3, textTransform: "uppercase" }}>{label}</div>
              <input type={type} value={formRata[k]} onChange={e => setFormRata(p => ({ ...p, [k]: e.target.value }))} style={{ background: `${C.navy}66`, border: `1px solid ${C.gold}33`, borderRadius: 8, padding: "7px 9px", color: C.ivory, fontSize: 12, outline: "none", width: "100%", fontFamily: "monospace" }} />
            </div>
          ))}
        </div>
    ...
    {enrichedRate.length === 0 && !showFormRata && <div style={{ padding: "20px 14px", textAlign: "center", color: C.grey, fontSize: 12 }}>Nessuna rata in corso</div>}
        ...
        <div key={r.id} style={{ borderBottom: i < enrichedRate.length - 1 ? `1px solid ${C.navy}44` : "none" }}>
          <div style={{ ...rowStyle, flexWrap: "wrap", gap: 8 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 10, color: C.grey, marginTop: 2 }}>Rata <strong style={{ color: C.ivory }}>{r.ratePagate + 1}</strong>/{r.rateTot} · addebito il <strong style={{ color: u.col }}>{r.giorno}</strong></div>
            </div>
            <span style={badgeStyle(u.bg, u.col)}>{u.label}</span>
            <div style={{ textAlign: "right", flexShrink: 0 }}>
...
const [formNuovo, setFormNuovo] = useState({ nome: “”, target: “”, emoji: “🎯”, colore: COLORI_FONDI[0], note: “” });
...
const fondoId = r[1];
if (fondiMap[fondoId]) {
fondiMap[fondoId].versamenti.push({ data: r[2], importo: parseFloat(r[3]) || 0, nota: r[4] || “” });
}
}
});
...
const salvaTuttiSuSheets = async (nuoviFondi) => {
...
const nuovoFondo = { id: `f${Date.now()}`, nome: formNuovo.nome, target: parseFloat(formNuovo.target), emoji: formNuovo.emoji, colore: formNuovo.colore, versamenti: [] };
const nuoviFondi = […fondi, nuovoFondo];
setFondi(nuoviFondi);
setShowNuovo(false);
setFormNuovo({ nome: “”, target: “”, emoji: “🎯”, colore: COLORI_FONDI[0], note: “” });
try {
await salvaTuttiSuSheets(nuoviFondi);
notify(“✅ Fondo creato”);
} catch (e) { notify(“❌ “ + e.message); }
};
...
```
  ...
  <div style={{ ...card(), display: "flex", justifyContent: "space-between", alignItems: "center" }}>
    ...
    <div style={{ textAlign: "right" }}>
      <div style={{ fontSize: 28, fontWeight: 900, color: totalTargetAll > 0 ? (totalSavedAll / totalTargetAll > 0.8 ? C.green : C.gold) : C.grey, fontFamily: "monospace" }}>
  ...
  {showNuovo && (
    <div style={{ ...card(), border: `1px solid ${C.gold}44` }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <input type="number" placeholder="Obiettivo €" value={formNuovo.target} onChange={e => setFormNuovo(p => ({ ...p, target: e.target.value }))} style={{ ...inp(), fontFamily: "monospace" }} />
        <div>
          <div style={{ fontSize: 9, color: C.grey, marginBottom: 6, textTransform: "uppercase" }}>Scegli icona</div>
        ...
        {isExpanded && (
          <div style={{ background: `${colore}08`, borderTop: `1px solid ${colore}22` }}>
            ...
            <div style={{ display: "flex", gap: 8, padding: "12px 14px 0" }}>
              ...
              <button onClick={() => eliminaFondo(fondo.id)} style={{ background: `${C.red}15`, border: `1px solid ${C.red}33`, borderRadius: 8, padding: "9px 12px", color: C.red, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                🗑️
              </button>
            </div>

            {/* Form versamento inline */}
            {showVersamento && fondoSelezionato === fondo.id && (
              <div style={{ margin: "12px 14px", padding: "12px", background: `${C.dark}88`, borderRadius: 10, border: `1px solid ${colore}33` }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <input type="number" placeholder="Importo €" value={formVersamento.importo} onChange={e => setFormVersamento(p => ({ ...p, importo: e.target.value }))} style={{ ...inp({ fontFamily: "monospace" }) }} />
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={aggiungiVersamento} style={{ flex: 1, background: `linear-gradient(135deg,${colore},${colore}88)`, border: "none", borderRadius: 8, padding: "9px", color: C.dark, fontWeight: 800, fontSize: 12, cursor: "pointer" }}>
                      💾 SALVA
                    </button>
                    <button onClick={() => setShowVersamento(false)} style={{ background: "none", border: `1px solid ${C.grey}44`, borderRadius: 8, padding: "9px 12px", color: C.grey, fontSize: 12, cursor: "pointer" }}>
                      Annulla
                    </button>
                  </div>
                </div>
...
generato_il: r[3], dati: JSON.parse(r[4] || “{}”)
}));
...
// Genera dati report per un mese/anno specifico
const generaDatiReport = (mese, anno) => {
...
const generaReport = async (mese, anno) => {
setGenerando(true);
try {
const dati = generaDatiReport(mese, anno);
const id = `rep-${mese}-${anno}`;
const generato_il = new Date().toISOString();
const nuovoReport = { id, mese, anno, generato_il, dati };

```
  // Rimuovi eventuale report esistente dello stesso mese
  const filtrati = reports.filter(r => !(r.mese === mese && r.anno === anno));
  const nuoviReports = [nuovoReport, ...filtrati].sort((a, b) => b.anno - a.anno || b.mese - a.mese);
...
// Auto-genera report del mese precedente al primo accesso del mese
useEffect(() => {
if (!token || spese.length === 0) return;
const oggi = new Date();
const mesePrecedente = oggi.getMonth() === 0 ? 11 : oggi.getMonth() - 1;
const annoPrecedente = oggi.getMonth() === 0 ? oggi.getFullYear() - 1 : oggi.getFullYear();
const esiste = reports.some(r => r.mese === mesePrecedente && r.anno === annoPrecedente);
if (!esiste && reports.length >= 0 && !loading) {
const hasSpese = spese.some(s => { const d = new Date(s.data); return d.getMonth() === mesePrecedente && d.getFullYear() === annoPrecedente; });
if (hasSpese) generaReport(mesePrecedente, annoPrecedente);
}
// eslint-disable-next-line react-hooks/exhaustive-deps
}, [token, spese.length, reports.length, loading]);

const scaricaPDF = (report) => {
const { dati } = report;
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .badge { font-size: 10px; font-weight: 800; letter-spacing: 0.2em; color: #C9A84C; text-transform: uppercase; margin-bottom: 8px; }
  .title { font-size: 32px; font-weight: 900; color: #F5F0E8; margin-bottom: 4px; }
  .subtitle { font-size: 13px; color: #8892A0; }
  .grid2 { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-bottom: 20px; }
  .grid4 { display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 10px; margin-bottom: 20px; }
  ...
  .kpi-value { font-size: 22px; font-weight: 900; font-family: monospace; }
  ...
  .top3-item { display: flex; justify-content: space-between; padding: 8px 12px; background: #1B2A4A55; border-radius: 8px; margin-bottom: 6px; }
  .delta { font-size: 11px; font-weight: 800; padding: 3px 8px; border-radius: 8px; }
  .alert-box { background: linear-gradient(135deg, #E74C3C18, #0D1B2A); border: 1px solid #E74C3C44; border-radius: 12px; padding: 14px; margin-bottom: 18px; }
...
<div class="header">
  ...
  <div class="subtitle">Generato il ${new Date(report.generato_il).toLocaleDateString("it-IT")} · ${dati.totaleTransazioni} transazioni registrate</div>
</div>

<div class="grid4">
  <div class="kpi">
    <div class="kpi-label">Entrate</div>
    <div class="kpi-value" style="color:#2ECC71">${eur(dati.entrate)}</div>
  </div>
  <div class="kpi">
    <div class="kpi-label">Uscite totali</div>
    <div class="kpi-value" style="color:#E74C3C">${eur(dati.totaleUscite)}</div>
  </div>
  <div class="kpi">
    <div class="kpi-label">Saldo netto</div>
    <div class="kpi-value" style="color:${dati.saldo >= 0 ? "#2ECC71" : "#E74C3C"}">${eur(dati.saldo)}</div>
  </div>
  <div class="kpi">
    <div class="kpi-label">Tasso risparmio</div>
    <div class="kpi-value" style="color:${dati.tassoRisparmio >= 20 ? "#2ECC71" : dati.tassoRisparmio >= 10 ? "#C9A84C" : "#E74C3C"}">${dati.tassoRisparmio.toFixed(1)}%</div>
  </div>
</div>
...
<div class="grid2">
  <div class="kpi">
    ...
    <div class="kpi-value" style="color:#3498DB;font-size:18px">${eur(dati.mediaGiorn)}/giorno</div>
  </div>
  <div class="kpi">
    <div class="kpi-label">Variazione vs mese prec.</div>
    <div class="kpi-value" style="color:${dati.deltaGlobale > 5 ? "#E74C3C" : dati.deltaGlobale < -5 ? "#2ECC71" : "#C9A84C"};font-size:18px">${dati.deltaGlobale > 0 ? "+" : ""}${dati.deltaGlobale.toFixed(1)}%</div>
  </div>
</div>

${dati.catFuoriControllo ? `

<div class="alert-box">
  <div style="font-size:12px;font-weight:800;color:#E74C3C;margin-bottom:4px">⚠️ Categoria fuori controllo</div>
  <div style="font-size:13px;color:#F5F0E8">${dati.catFuoriControllo.emoji} <strong>${dati.catFuoriControllo.label}</strong> — spesa aumentata del <strong style="color:#E74C3C">+${dati.catFuoriControllo.delta.toFixed(0)}%</strong> rispetto al mese precedente (${eur(dati.catFuoriControllo.valore)} totali)</div>
</div>` : ""}
...
<div class="section">
  ...
  ${CATEGORIE_DEF.filter(c => dati.aggMese[c.id] > 0).sort((a, b) => dati.aggMese[b.id] - dati.aggMese[a.id]).map(c => {
    ...
    return `<div class="cat-row">
      ...
      <span style="flex:1;font-size:13px;font-weight:700;color:#F5F0E8">${c.label}</span>
      ...
      ${delta !== null ? `<span class="delta" style="background:${delta > 5 ? "#E74C3C18" : delta < -5 ? "#2ECC7118" : "#C9A84C18"};color:${delta > 5 ? "#E74C3C" : delta < -5 ? "#2ECC71" : "#C9A84C"}">${delta > 0 ? "▲" : "▼"} ${Math.abs(delta).toFixed(0)}%</span>` : ""}
    </div>`;
  }).join("")}
</div>
...
<div class="section">
  ...
  <div class="top3-item">
    <div>
      <div style="font-size:13px;font-weight:700;color:#F5F0E8">${["🥇","🥈","🥉"][i]} ${s.descrizione}</div>
      <div style="font-size:10px;color:#8892A0">${s.data} · ${CAT_MAP[s.categoria]?.label || s.categoria}</div>
    </div>
    <span style="font-size:16px;font-weight:900;color:#E74C3C;font-family:monospace">${eur(Math.abs(s.importo))}</span>
  </div>`).join("")}
</div>` : ""}

<div class="section">
  <div class="sh"><div class="sh-line"></div><div class="sh-title">Confronto mese precedente → ${MESI[dati.mese]}</div><div class="sh-fill"></div></div>
  ${[
...
<div class="footer">Budget HQ · Angelo · ${new Date().getFullYear()}</div>
</body></html>`;
...
a.href = url;
a.download = `BudgetHQ_Report_${MESI[dati.mese]}_${dati.anno}.html`;
a.click();
URL.revokeObjectURL(url);
notify("📄 Report scaricato — aprilo nel browser e stampa come PDF");
```
...
```
  ...
  <div style={{ ...card() }}>
    ...
    <p style={{ fontSize: 12, color: C.grey, lineHeight: 1.7, margin: 0 }}>
      I report vengono generati automaticamente al primo accesso del mese successivo e salvati su Google Sheets. Puoi anche generarli manualmente per qualsiasi mese.
    </p>
  </div>
  ...
  <div style={{ ...card(), border: `1px solid ${C.gold}33` }}>
    ...
    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
      <select defaultValue={mesePrecedente} id="sel-mese-report" style={{ ...inp({ flex: 1 }), background: C.navy }}>
      ...
      <button
        onClick={() => {
          const m = parseInt(document.getElementById("sel-mese-report").value);
          const a = parseInt(document.getElementById("sel-anno-report").value);
          generaReport(m, a);
        }}
        disabled={generando}
        style={{ background: `linear-gradient(135deg,${C.gold},#9A7830)`, border: "none", borderRadius: 8, padding: "9px 14px", color: C.dark, fontWeight: 800, fontSize: 12, cursor: "pointer", flexShrink: 0, opacity: generando ? 0.6 : 1 }}>
        {generando ? "⏳" : "Genera"}
      </button>
    </div>
  </div>
  ...
  {!loading && reports.length === 0 && (
    <div style={{ textAlign: "center", padding: "40px 20px", color: C.grey }}>
      ...
      <div style={{ fontSize: 12, marginTop: 6 }}>Il primo report verrà generato automaticamente</div>
    </div>
  )}
  ...
  {reports.map(report => {
    ...
    const tassoColor = dati.tassoRisparmio >= 20 ? C.green : dati.tassoRisparmio >= 10 ? C.gold : C.red;
    ...
    return (
      <div key={report.id} style={{ borderRadius: 12, overflow: "hidden", border: `1px solid ${isAperto ? C.gold + "55" : C.gold + "18"}` }}>
        ...
        <div onClick={() => setReportAperto(isAperto ? null : report)} style={{ background: `linear-gradient(135deg,${C.dark},${C.navy})`, padding: "14px", cursor: "pointer" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <span style={{ fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 8, background: `${tassoColor}18`, color: tassoColor }}>
              💰 Risparmio {dati.tassoRisparmio?.toFixed(1)}%
            </span>
            <span style={{ fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 8, background: `${dati.deltaGlobale > 0 ? C.red : C.green}18`, color: dati.deltaGlobale > 0 ? C.red : C.green }}>
              {dati.deltaGlobale > 0 ? "▲" : "▼"} {Math.abs(dati.deltaGlobale)?.toFixed(0)}% vs prec.
            </span>
          </div>
        </div>
        ...
        {isAperto && (
          <div style={{ background: `${C.navy}44`, borderTop: `1px solid ${C.gold}22` }}>
            ...
            {/* Bottone scarica */}
            <div style={{ padding: "0 14px 14px" }}>
              <button onClick={() => scaricaPDF(report)} style={{ width: "100%", background: `linear-gradient(135deg,${C.gold},#9A7830)`, border: "none", borderRadius: 10, padding: "12px", color: C.dark, fontWeight: 800, fontSize: 13, cursor: "pointer" }}>
                📄 SCARICA REPORT (HTML → stampa come PDF)
              </button>
            </div>
          </div>
        )}
...
setSpese((sr.values || []).map((r, i) => ({ id: `sh-${i}`, data: r[0] || “”, descrizione: r[1] || “”, categoria: r[2] || “altro”, importo: parseFloat(r[3]) || 0, tipo: r[4] || “”, note: r[5] || “”, sheetRow: i + 2 })));
...
} catch (e) { if (!quiet) notify("❌ " + e.message); }
...
const importoFinal = form.tipo === “Entrata” ? Math.abs(parseFloat(form.importo)) : -Math.abs(parseFloat(form.importo));
...
Ogni modifica nell’app va su <strong style={{ color: C.gold }}>Google Sheets</strong> in tempo reale.
</div>
  ...
  <div style={{ background: `${C.dark}F2`, borderBottom: `1px solid ${C.gold}33`, padding: "13px 16px 0", position: "sticky", top: 0, zIndex: 100 }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 11 }}>
      <div>
        ...
        <div style={{ fontSize: 17, fontWeight: 900, color: C.ivory }}>{userInfo ? `Ciao, ${userInfo.given_name} 👋` : "💰 Dashboard"}</div>
      </div>
      <div style={{ display: "flex", gap: 7, alignItems: "center" }}>
        ...
        <button onClick={() => loadData()} disabled={loading} style={{ background: `${C.navy}88`, border: `1px solid ${C.gold}33`, borderRadius: 8, padding: "5px 9px", color: C.gold, cursor: "pointer", fontSize: 13 }}>{loading ? "⏳" : "🔄"}</button>
    ...
    <div style={{ display: "flex", overflowX: "auto", scrollbarWidth: "none" }}>
      {TABS.map(t => (
        <button key={t.id} onClick={() => setTab(t.id)} style={{
          flexShrink: 0, padding: "7px 10px", border: "none", cursor: "pointer", background: "transparent",
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              ...
              <input type="number" value={stipendio || ""} placeholder="0.00"
                onChange={e => setStipendio(parseFloat(e.target.value) || 0)}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
        ...
        <div style={card()}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <button onClick={aggiungiSpesa} disabled={loading} style={{ background: `linear-gradient(135deg,${C.gold},#9A7830)`, border: "none", borderRadius: 8, padding: "11px", color: C.dark, fontWeight: 800, fontSize: 12, cursor: "pointer", opacity: loading ? .6 : 1 }}>
